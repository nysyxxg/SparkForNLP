...
object DealTry {
	def main(args: Array[String]): Unit = {
		...
		hbase.set(TableInputFormat.INPUT_TABLE, "test") //
		//hbase.set(TableInputFormat.SCAN, "1430203560000")
		//hbase.set(TableInputFormat.SCAN_ROW_START, "146")
		//hbase.set(TableInputFormat.SCAN_ROW_STOP, "147")

		//零件二：获取数据
		val hbaseRDD = sc.newAPIHadoopRDD(hbase, classOf[TableInputFormat], classOf[ImmutableBytesWritable], classOf[Result])
		...
		//零件四：分词
		val data: DataFrame = hbaseRDD.map(
			line => (
				//读取取Hbase中文章的ID
				Bytes.toString(line._1.get()),"13", {
				//读取Hbase中文章的内容
				val content = Bytes.toString(line._2.getValue(Bytes.toBytes("article"), Bytes.toBytes("content")))
				//去除文章内容中的Html标签
		...
		//零件五：打印前5条预览
		data.select("id", "words").take(5).foreach(println)
	data.select("lable","words").rdd.saveAsTextFile("file:///e://svm//zhengzhi/")
	/*	//零件六：转为TF-IDF
