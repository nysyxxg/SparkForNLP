...
object LoadModel {
	...
	def pric(str: String,dictPath:String,stopwordPath:String, tfidf1: PipelineModel, tfidf2: PipelineModel, svm1: SVMModel, svm2: SVMModel): java.util.List[String] = {
		...
		sc.textFile(dictPath)
			.map(_.split("	")).collect()
			.foreach(attributes => UserDefineLibrary.insertWord(attributes(0), attributes(1), 1000))
		//(2)加载停用词词表
		val stopwords: java.util.List[String] = sc.textFile(stopwordPath).collect().toSeq
