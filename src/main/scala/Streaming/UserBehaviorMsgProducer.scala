...
/**
	* Created by MingDong on 2017/2/14.
	*/
...
object UserBehaviorMsgProducerClient {
	def main(args: Array[String]) {
		/*if (args.length < 2) {
			println("Usage:UserBehaviorMsgProducerClient 172.16.110.110:9092 user-behavior-topic")
			System.exit(1)
		}*/
		//start the message producer thread
		new Thread(new UserBehaviorMsgProducer("slave4:9092","user-behavior-topic")).start()
	}
}
