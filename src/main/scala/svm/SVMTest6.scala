...
object SVMTest6 {
	...
	def main(args: Array[String]): Unit = {
		...
		var str = " 昨夜 22点 40分 更换 工作 正式 启动 此次 更换 动用  两辆 吊车 一辆 货车 更换 工作 开始 后 " +
			"一辆 吊车 首先 从 天安门 城楼  门洞 中 缓缓 驶出 几名 施工 人 员 随即 登 上 城楼，" +
			"探身 下 到 悬挂 毛主席 像 的 位置 处，小心 翼翼 地 将 悬挂 一年 的 旧画 像 轻轻 摘下，并 用 吊车 吊离 悬挂 位置";

		var tRDD = sc.parallelize(Seq(str))
		var tDF = tRDD.map(line=>
			("1,1,0",line.split(" "))
		).toDF("lable","message")
	var tVectors =	model.transform(tDF)
		TestModel(smodel1,tVectors,0).collect().foreach(println)
		TestModel(smodel1,tVectors,1).collect().foreach(println)
		TestModel(smodel1,tVectors,2).collect().foreach(println)
	}
	...
	def getModel1(dataDF: DataFrame): PipelineModel = {
		...
		val pipeline = new Pipeline().setStages(Array(labelIndexer, hashingTF, idf, selector))
