...
/**
	* 用TF-IDF来向量化数据做特征完成的完整版SVM情感分类-2，判断正负面
...
object SVMTest7Plus {
	...
	def main(args: Array[String]): Unit = {
		...
		val parsedRDD = sc.textFile("file:///e:/svm/svm5.txt").map {
			x =>
				val data = x.split(",")
		...
		val Array(training1, test1) = msgDF.randomSplit(Array(0.9, 0.1), seed = 12L)

		val model = getModel1(training1)
		model.save("file:///e:/svm/tfidfModel2/")
		//向量化语料集
		...
		smodel1.save(sc,"file:///e:/svm/svmModel2/")
			//得到正负面的预测值和真实值（预测值，真实值）
